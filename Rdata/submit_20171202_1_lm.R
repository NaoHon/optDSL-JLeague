...
#timeInt_cの加工(timeIntをxで折り返し)
F_timeInt_c2 <- function(df,x) {
  df<-df %>%
    dplyr::mutate(timeInt_c2=abs(x-timeInt_c))  #absは絶対値
  return(df)
}

#yearの加工(timeIntをxで折り返し)
F_year2 <- function(df,x) {
  df<-df %>%
    dplyr::mutate(year2=abs(x-year))  #absは絶対値
  return(df)
}
...
###CSV出力(ヘッダーなし)
write.table(submit_all, file="C:/study/JLeague/submit/submit_20171202_1_lm.csv",
...
F_linearplot2(train_new,"home_lank2") %>% plot()
F_linearplot2(train_new,"away_lank2") %>% plot()
F_linearplot2(tmpJ1,"home_lank2") %>% plot()
F_linearplot2(tmpJ1,"away_lank2") %>% plot()
F_linearplot2(tmpJ2,"home_lank2") %>% plot()
F_linearplot2(tmpJ2,"away_lank2") %>% plot()
