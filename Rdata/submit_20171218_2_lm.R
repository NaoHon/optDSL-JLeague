...
F_pre <- function(data,data_condition,data_stadium,traindataflag) {
  ...
  #同一都道府県のチーム数
  df_kenNum <- data_stadium_tmp %>%
    dplyr::group_by(address_ken) %>%
    dplyr::summarize(address_kenNum = n())
  data_stadium_tmp <- merge(data_stadium_tmp, df_kenNum, by = "address_ken",all = TRUE)
  
  #住所をGeocoding APIのURLに変換
  #  for(i in 1:nrow(data_stadium_tmp)){
# 
#   address <-data_stadium_tmp$address[i]
...
#   # JSONからRに緯度・経度の情報を抽出
#   rjson <- fromJSON(json)
#   data_stadium_tmp$latitude[i] <- rjson$results[[1]]$geometry$location$lat
#   data_stadium_tmp$longitude[i] <- rjson$results[[1]]$geometry$location$lng
#   }
  ...
  #不快指数
  data_condition_tmp$humidity  <- data_condition_tmp$humidity %>%
    gsub(pattern="%", replacement="", fixed = TRUE)
  data_condition_tmp$humidity  <- as.integer(data_condition_tmp$humidity)

  data_condition_tmp$di <- 0.81*data_condition_tmp$temperature+0.01*data_condition_tmp$humidity*(0.99*data_condition_tmp$temperature-14.3)+46.3
...
#address_kenNumを加工
F_linearplot2(lm_train_j1,"address_kenNum") %>% plot()
F_linearplot2(lm_train_j2,"address_kenNum") %>% plot()
lm_train_j1<-lm_train_j1 %>% dplyr::mutate(address_kenNum2=abs(3-address_kenNum)*-1) 
lm_train_j2<-lm_train_j2 %>% dplyr::mutate(address_kenNum2=abs(4-address_kenNum)*-1) 
F_linearplot2(lm_train_j1,"address_kenNum2") %>% plot()
F_linearplot2(lm_train_j2,"address_kenNum2") %>% plot()

lm_test_j1<-lm_test_j1 %>% dplyr::mutate(address_kenNum2=abs(3-address_kenNum)*-1) 
lm_test_j2<-lm_test_j2 %>% dplyr::mutate(address_kenNum2=abs(4-address_kenNum)*-1) 
...
lm_train_j1$timeInt_c2 <- ifelse(lm_train_j1$timeInt_c2 == -6, -14, lm_train_j1$timeInt_c2)
lm_test_j1$timeInt_c2 <- ifelse(lm_test_j1$timeInt_c2 == -6, -14, lm_test_j1$timeInt_c2)
F_linearplot2(lm_train_j1,"timeInt_c2") %>% plot()

lm_train_j2$timeInt_c2 <- ifelse(lm_train_j2$timeInt_c2 == -5, -12, lm_train_j2$timeInt_c2)
lm_test_j2$timeInt_c2 <- ifelse(lm_test_j2$timeInt_c2 == -5, -12, lm_test_j2$timeInt_c2)
F_linearplot2(lm_train_j2,"timeInt_c2") %>% plot()


#TVを加工
F_linearplot4(lm_train_j1,"tv_num") %>% plot()
F_linearplot4(lm_train_j2,"tv_num") %>% plot()


lm_train_j1$tv_num2 <- ifelse(lm_train_j1$tv_num == 5, 4.3, lm_train_j1$tv_num)
lm_test_j1$tv_num2 <- ifelse(lm_test_j1$tv_num == 5, 4.3, lm_test_j1$tv_num)

lm_train_j2$tv_num2 <- ifelse(lm_train_j2$tv_num == 1, -2, lm_train_j2$tv_num)
lm_test_j2$tv_num2 <- ifelse(lm_test_j2$tv_num == 1, -2, lm_test_j2$tv_num)

lm_train_j2$tv_num2 <- ifelse(lm_train_j2$tv_num2 == 5, 7, lm_train_j2$tv_num2)
lm_test_j2$tv_num2 <- ifelse(lm_test_j2$tv_num2 == 5, 7, lm_test_j2$tv_num2)

F_linearplot4(lm_train_j1,"tv_num2") %>% plot()
F_linearplot4(lm_train_j2,"tv_num2") %>% plot()
...
lm_train_j1_2<-dplyr::select(lm_train_j1, y3, tv_num2, capa, setu2, home,away, kyuujitu,home_lank2,away_lank2,timeInt_c2,address_kenNum2)
lm_train_j2_2<-dplyr::select(lm_train_j2, y3, tv_num2, capa, setu2, home,away, kyuujitu,home_lank2,away_lank2,timeInt_c2,address_kenNum2)
lm_test_j1_2<-dplyr::select(lm_test_j1, tv_num2, capa, setu2, home,away, kyuujitu,home_lank2,away_lank2,timeInt_c2,address_kenNum2)
lm_test_j2_2<-dplyr::select(lm_test_j2, tv_num2, capa, setu2, home,away, kyuujitu,home_lank2,away_lank2,timeInt_c2,address_kenNum2)

# lm_train_j1_2<-dplyr::select(lm_train_j1, y3, tv_num2)
# lm_train_j2_2<-dplyr::select(lm_train_j2, y3, tv_num2)
# lm_test_j1_2<-dplyr::select(lm_test_j1, tv_num2)
# lm_test_j2_2<-dplyr::select(lm_test_j2, tv_num2)

# ###重回帰分析
# #family:目的変数の確率分布とリンク関数の設定(今回は正規分布と恒等写像)
lm_j1<-glm(y3 ~ ., data=lm_train_j1_2, family=gaussian(link="identity"))
lm_j2<-glm(y3 ~ ., data=lm_train_j2_2, family=gaussian(link="identity"))
...
# 収容率の対数オッズから観客数に戻す
pred_j1 <- lm_test_j1_2$capa * (1 / (1 + exp(-pred_j1)))
pred_j2 <- lm_test_j2_2$capa * (1 / (1 + exp(-pred_j2)))
...
###CSV出力(ヘッダーなし)
write.table(submit_all, file="C:/study/JLeague/submit/submit_20171218_2_lm.csv",
...
# 収容率の対数オッズから観客数に戻す
pred_j1 <- lm_train_j1_2$capa * (1 / (1 + exp(-pred_j1)))
pred_j2 <- lm_train_j2_2$capa * (1 / (1 + exp(-pred_j2)))
...
#########変数選択 rmseを確認したいとき用２
lm_train_j1_2<-dplyr::select(lm_train_j1, y,tv_num2,address_kenNum2)
lm_train_j2_2<-dplyr::select(lm_train_j2, y,tv_num2,address_kenNum2)
...
F_linearplot4 <- function(df,group_by_key) {
  
  g_dat <- df %>%
    dplyr::group_by_(group_by_key) %>%
    dplyr::summarise(y_mean = mean(y3)) %>%
    dplyr::ungroup(.) %>%
    ggplot(., aes_(x = as.name(group_by_key), y = as.name("y_mean"))) + geom_line()
  return(g_dat)
...
zan<-data.frame(lm_train_all, Res=abs(lm_train_all$y-submit_all$pred), pred=submit_all$pred, 
  ...
  dplyr::select(id, Res, y, pred, dif, stage, home, away, stadium, capa, setu,address_kenNum) %>%
